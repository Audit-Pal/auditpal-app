...
// Quantum Particle class moved outside
class QuantumParticle implements Particle {
    ...
    draw() {
        ...
        ctx.fillStyle = `rgba(168, 85, 247, ${0.8 + this.quantumState * 0.2})`; // Use a theme color for the core
        ctx.fill();
    }
}
...
export function QuantumBackground() {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < particleCount; i++) {
            particles.push(new QuantumParticle(canvas, ctx));
        }
        ...
        const animate = () => {
            ...
            // Quantum field background - translucent base
            ctx.fillStyle = "rgba(0, 0, 0, 0.02)"; // Very light trail
    ...
    return (
        <div className="fixed inset-0 -z-10 pointer-events-none">
            <canvas
                ref={canvasRef}
                className={`absolute inset-0 bg-background ${glitchActive ? 'animate-pulse' : ''}`}
                style={{
