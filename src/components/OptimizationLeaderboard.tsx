...
import { EVMBenchSOTA } from "@/lib/evmbench-data";
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    if (benchmarkId === 'evm-bench') {
                        const evmSota = EVMBenchSOTA.map((sota, index) => ({
                            rank: index + 1,
                            previousRank: index + 1,
                            name: sota.name,
                            winRate: sota.overall,
                            wins: Math.floor(parseFloat(sota.overall) * 1.2), // Mock
                            attempts: 120,
                            rewards: index === 0 ? "Elite SOTA" : "Verified",
                            age: "Verified",
                            trend: [65, 68, 70, parseFloat(sota.overall), parseFloat(sota.overall), parseFloat(sota.overall)],
                            isTeam: true,
                            isSota: true
                        }));
                        mappedData = [...evmSota, ...mappedData].map((item, idx) => ({ ...item, rank: idx + 1 }));
                    }
        ...
        >
                            ...
                            >
                                ...
                                <div className="relative z-10 p-4 flex flex-col items-center justify-center">
                                    ...
                                    <div className="text-center w-full">
                                        <div className="mb-2">
                                            <h3 className="text-lg font-bold text-foreground tracking-tight flex items-center justify-center gap-1.5">
                                                {item.name}
                                                {item.isTeam && (
                                                    <span className="p-0.5 rounded-full bg-muted" title="Team">
                                        ...
                                        <div className="mt-2">
                                            <div className={cn(
                                                ...
                                                isFirst ? "text-kast-teal" : "text-foreground/80"
                                            )}>
                                                {item.winRate}
                                            </div>
                                            <div className="text-[9px] uppercase tracking-wider text-muted-foreground font-bold mt-[-2px]">Win Rate</div>
                                        </div>
                                    </div>
                                </div>
                            </DataModule>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-2 px-1 mb-6">
                <div className="flex items-center gap-4">
                    <div className="bg-muted h-12 w-12 flex items-center justify-center rounded-md shadow-[0_2px_12px_-3px_rgba(0,0,0,0.1)] border border-border text-foreground">
                ...
                <div className="relative w-full sm:w-72 group">
                    ...
                    <div className="relative bg-card rounded-md shadow-sm border border-border flex items-center px-4 py-2.5 transition-shadow group-hover:shadow-md">
                        ...
                        <input
                            ...
                            className="w-full bg-transparent border-none p-0 text-sm focus:ring-0 focus:outline-none text-foreground placeholder:text-muted-foreground font-medium"
                        />
                    </div>
                </div>
            </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-muted/50 border-b border-border">
                                <th className="px-6 py-4 text-[11px] font-bold text-muted-foreground uppercase tracking-widest w-16 text-center">Δ</th>
                                <th className="px-6 py-4 text-[11px] font-bold text-muted-foreground uppercase tracking-widest w-16">#</th>
                                ...
                                <th className="px-6 py-4 text-[11px] font-bold text-muted-foreground uppercase tracking-widest text-center">Attempts</th>
                                <th className="px-6 py-4 text-[11px] font-bold text-muted-foreground uppercase tracking-widest text-center">Rewards</th>
                                <th className="px-6 py-4 text-[11px] font-bold text-muted-foreground uppercase tracking-widest text-center">Age</th>
                                ...
                                <tr key={item.rank} className="group hover:bg-muted/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-9 h-9 rounded-full bg-muted border border-border flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform">
                                                {item.rank === 1 ? <Crown className="w-4 h-4 text-amber-500" /> :
                                                    <span className="text-xs font-bold text-muted-foreground">{item.name.substring(0, 2).toUpperCase()}</span>}
                                            </div>
                                            <div className="flex flex-col">
                                                <span className="text-sm font-bold text-foreground group-hover:text-kast-teal transition-colors" title={item.hotkey}>
                                                    {item.name}
                                                </span>
                                                <span className="text-[10px] text-muted-foreground font-mono truncate max-w-[120px]" title={item.hotkey}>
                                                    {item.hotkey ? `${item.hotkey.substring(0, 6)}...${item.hotkey.substring(item.hotkey.length - 4)}` : 'No hotkey'}
                                                </span>
                                                {item.isTeam && <span className="text-[10px] text-muted-foreground flex items-center gap-1"><Users className="w-3 h-3" /> Team</span>}
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        <span className="text-sm font-mono text-muted-foreground">{item.wins}</span>
                                    </td>
                                    <td className="px-6 py-4 text-center">
                                        <span className="text-sm font-mono text-muted-foreground">{item.attempts}</span>
                                    </td>
                                    <td className="px-6 py-4 text-center">
                                        <span className="text-sm font-mono text-indigo-400 font-bold">{item.rewards}</span>
                                    </td>
                                    <td className="px-6 py-4 text-center">
                                        <span className="text-sm font-mono text-muted-foreground">{item.age}</span>
                                    </td>
                                    <td className="px-6 py-4">
